...
`);
	...
	stripjunk(str) {
		str = str.replaceAll('[–]', '');
		str = str.replaceAll(/@UUID\[[^\]]+\]/g, '');
		str = str.replaceAll(/\[\[\/r\s+([^\]]+)\]\]/g, '$1'); // eliminate dice rolls.
		// @UUID[Actor.coeOtqGlZeyowS3Z.Item.WLotMtdHJ7ruERiN]{Flight}
		str = str.replaceAll(/@[a-zA-Z]+\[([^\]|]+)(|[^\]])*\]/g, '$1');
	...
	biography(a) {
		...
		let bio = this.stripjunk(a.system.details.biography.value);
		if (bio)
			this.dw(`<div>${bio}</div>`);
	...
	displayitems(a, title, type, showDetails) {
		...
		items.forEach((i) => {
			...
			if (i.system.activation != undefined && i.system.activation.type) {
				a = i.system.activation;
				if (a.type == 'special')
					deets = this.cat(deets, ', ', a.type);
				else if (a.type != 'action')
					deets = this.cat(deets, ', ', `${a.cost != 1 ? a.cost + ' ' : ''}${a.type} action${a.cost != 1 ? 's' : ''}`);
			}
        ...
        // },
			...
			if (showDetails && i.system.description.value != '') {
				...
				this.dw(this.stripjunk(i.system.description.value));
				this.dw(`</p>\n`);
			}
		});
	}
	...
	exportStatblock(a) {
		...
		// if (a.type !== 'character') {
			// ui.notifications.warn("Not a character: " + name);
			// return;
		// }
  ...
  <tr>
		...
		if (showDetails) {
			...
			cl.forEach((f) => {
				...
				if (showDetails && f.system.description.value != '') {
					this.dw(`<p class="desc">\n`);
					this.dw(this.stripjunk(f.system.description.value));
					this.dw(`</p>\n`);
				}
			});
		}
		...
		if (weap.length > 0) {
			...
			weap.forEach((w) => {
				...
				if (showDetails && w.system.description.value != '') {
					this.dw(`<p class="desc">\n`);
					this.dw(this.stripjunk(w.system.description.value));
					this.dw(`</p>\n`);
				}
			});
		}
	...
	spelldetails(s) {
		...
		this.dw('<div class="top-margin">' + this.stripjunk(s.system.description.value) + "</div>");
	}
    ...
    if (game.system.id == "dnd5e") {
        entries.push({
            ...
            condition: li => {
                ...
                const canPolymorph = actor.isOwner || game.user.isGM;
                return canPolymorph;
            },
            callback: async (li) => {
				const actor = game.actors.get(li.data("documentId"));
